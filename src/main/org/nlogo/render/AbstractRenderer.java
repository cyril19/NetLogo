...
public abstract strictfp class AbstractRenderer
  ...
  public void paint(GraphicsInterface g, ViewSettings settings) {
    ...
    if (world instanceof org.nlogo.mirror.FakeWorld) {
      /*
       * This instanceof check is a fairly hugly hack. Hopefully, it will be
       * replaced at some point down the road. If we do change it in some
       * appropriate way, consider removing the dependency from render to mirror
       * in Depends.scala. NP 2012-07-11
       */
      org.nlogo.mirror.FakeWorld w = (org.nlogo.mirror.FakeWorld) world;
      if (w.trailDrawing().isDefined()) {
        try {
          _trailDrawer.readImage(new java.io.ByteArrayInputStream(w
              .trailDrawing().get()));
        } catch (java.io.IOException e) {
          /*
           * I don't like to swallow the exception, but I'm not sure what else
           * do to. Since we are using a ByteArrayInputStream, it should be
           * relatively safe that no exception occurs... but still... ? NP
           * 2012-07-11
           */
        }
      ...
      if (!_trailDrawer.drawingBlank
          || (WINDOWS && anyTurtles() && !settings.isHeadless())) {
        topology.paintViewImage(g, _trailDrawer.getAndCreateDrawing(false));
      }
    }
