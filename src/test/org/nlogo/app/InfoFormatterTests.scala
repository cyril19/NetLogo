...
class InfoFormatterTests extends FunSuite {
  ...
  test("read function that should probably be in a utils class somewhere") {
    def process(s: String) =
      expectResult(s) { read(new ByteArrayInputStream(s.getBytes)) }
    process("goo")
    process("\ngoo")
    process("\ngoo\n")
  }
  ...
  test("< characters get converted to &lt;") {
    expectResult("""<p>is 5&lt;6?<br/>is 5 &lt; 6?<br/>is 5 &lt; x?</p>""") {
      toInnerHtml("""|is 5<6?
                     |is 5 < 6?
                     |is 5 < x?""".stripMargin)
    }
  ...
  test("table parsing makes no changes and isn't outrageously slow") {
    ...
    expectResult(content) { toInnerHtml(content) }
