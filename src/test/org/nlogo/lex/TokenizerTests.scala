...
class TokenizerTests extends FunSuite {
  ...
  def tokenize(s: String) = {
    ...
    expectResult(TokenType.EOF)(result.last.tyype)
  ...
  test("OneUnderscore1") {
    ...
    expectResult("Token(_,BAD,This non-standard character is not allowed.)")(
      tokens.mkString)
  }
  test("OneUnderscore2") {
    val tokens = tokenizeRobustly("_\n")
  ...
  test("ThreeUnderscores1") {
    ...
    expectResult("Token(__magic-open,COMMAND,_magicopen)")(
      tokens.mkString)
  }
